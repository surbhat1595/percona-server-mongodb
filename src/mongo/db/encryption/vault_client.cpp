...
std::optional<SecretMetadata> VaultClient::Impl::requestSecretMetadata(
    ...
    BSONObj bson = fromjson(replyBody);
...
std::uint64_t VaultClient::Impl::putKey(const std::string& secretPath,
    ...
    static constexpr std::string_view kVersionMismatchErrorMsg =
    ...
    for (unsigned remainingAttemptCount = _checkMaxVersions ? 3 : 1;;) {
        ...
        if (--remainingAttemptCount > 0 && reply.code == kHttpStatusCodeBadRequest &&
            std::string_view(replyBody).find(kVersionMismatchErrorMsg) != std::string_view::npos) {
        ...
        return bsonObjectGetNestedValue<PositiveUint64>(fromjson(replyBody), "data.version");
    }
    MONGO_UNREACHABLE;
}
