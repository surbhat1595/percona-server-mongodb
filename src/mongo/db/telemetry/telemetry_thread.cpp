...
public:
    ...
    void run() final {
        const ThreadClient tc(name(), getGlobalServiceContext()->getService());
...
private:
    Status _initParameters(ServiceContext* serviceContext) {
        ...
        const auto nss = NamespaceString::makeLocalCollection(kTelemetryCollection);
    ...
    // advance nextScrape and store it into kTelemetryCollection
